...
import { getResearchers, getBuilders, getBuilderProjects, getResearcherPapers } from "@/lib/data";
...
export default function DirectoryPage() {
  ...
  const allDomains = Array.from(
    new Set([
      ...researchers.flatMap((r) => r.domains),
      ...getBuilderProjects("").length ? [] : builders.flatMap((b) => {
        const projects = getBuilderProjects(b.id);
        return projects.flatMap((p) => p.domains);
      }),
    ])
  ).sort();

  // Recalculate domains properly
  ...
  const filteredResearchers = researchers.filter((r) => {
    ...
    const matchesDomain = domainFilter === "all" || r.domains.includes(domainFilter as any);
  ...
  const filteredBuilders = builders.filter((b) => {
    ...
    const matchesDomain =
      domainFilter === "all" ||
      getBuilderProjects(b.id).some((p) => p.domains.includes(domainFilter as any));
    return matchesSearch && matchesDomain;
  });
      ...
      {tab === "researchers" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredResearchers.map((r) => {
            const paperCount = getResearcherPapers(r.id).length;
            return (
              <Link
                key={r.id}
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-white/40 mt-1">{projects.length} project{projects.length !== 1 ? "s" : ""}</p>
                    <div className="flex flex-wrap gap-1 mt-2">
